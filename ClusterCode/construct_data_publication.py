...
for i in [0,1]:
    ...
    season = range(12)
    time_values = np.zeros(len(month_ind))
    for k in range(26,29):
        for l in range(-4,2):
            if not (k == 26 and l == -4):
                mask_count = 0
                if i == 0:
                    this_data = data.variables[descriptions[i]][:, 0, k, l]
                else:
                ...
                if i==0:
                    pd.Series(np.array([sum([this_data.data[12 * i + j] for j in season])/len(season) for i in range(math.floor(len(this_data.data)/12))]), year_ind).to_csv("Processed/Cell Data/seasonal_data_" + descriptions[i] + "lat" + str(k) + "lon" + str(l))
                else:
                    pd.Series(1-np.array([sum([this_data.data[12 * i + j] for j in season]) / len(season) for i in range(math.floor(len(this_data.data) / 12))]), year_ind).to_csv("Processed/Cell Data/seasonal_data_" + descriptions[i] + "lat" + str(k) + "lon" + str(l))
                print(mask_count)
    time_values = time_values/17
    if i == 1:
